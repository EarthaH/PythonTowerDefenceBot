...
class StarterBot:
    ...
    def shouldDefend(self):
        ...
        for col in range(8):
            if self.checkAttack(col) and not self.checkFirstDefense(col):
                return col,1
            if self.checkMultiAttack(col) and not self.checkSecondDefense(col):
                ret_col = col
    ...
    def checkTotalAttackBuildings(self):
        total = 0
        for i in range(8):
            total += self.player_buildings[i].count(1)
    ...
    def checkMultiAttack(self, lane_number):
    ...
    def generateAction(self):
        ...
        if t == 0:
            ...
            if self.checkTotalEnergyBuildings() < 4 and self.energy >= 20:
                x,y = self.openEnergyCell()
                building = 2
            elif self.checkTotalAttackBuildings() < 1 and self.energy >= 30:
                x,y = self.openAttackCell()
                building = 1
            elif self.checkTotalEnergyBuildings() < 9 and self.energy >= 20:
                x,y = self.openEnergyCell()
                building = 2
            elif self.energy >= 30:
                x,y = self.openAttackCell()
